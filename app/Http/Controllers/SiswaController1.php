...
class SiswaController extends Controller
{
    ...
    public function destroy(Siswa $siswa)
    {
        ...
        if ($siswa->qr_code) {
            $qrPath = str_replace('storage/', '', $siswa->qr_code);
    ...
    /**
     * Generate QR Code for the specified student
     */
    private function generateQrCode(Siswa $siswa)
    {
        ...
        $fileName = 'qr-code-siswa-' . $siswa->id . '.svg';
    ...
    /**
     * Download current student's QR code
     */
    ...
    public function attendanceTrack(Request $request)
    {
        ...
        $type = $request->query('type', 'gerbang'); // Default to gerbang
        $period = $request->query('period', 'all'); // Default to all
        ...
        if ($type === 'gerbang') {
            $query = AbsensiGerbang::where('related_id', $siswa->id);

            if ($startDate && $endDate) {
                $query->whereBetween('tanggal', [$startDate->toDateString(), $endDate->toDateString()]);
            }

            $absensi = $query->with(['scannedBy'])->orderBy('tanggal', 'desc')->paginate(10);
        } else {
            $query = AbsensiSiswaKelas::where('siswa_id', $siswa->id)
    ...
    public function exportAttendancePdf(Request $request)
    {
        ...
        if ($type === 'gerbang') {
            ...
            // Generate PDF
            $viewName = 'exports.student-gate-attendance-pdf';
        } else {
            $query = AbsensiSiswaKelas::where('siswa_id', $siswa->id)
                ->with(['jadwal.jadwalPelajaran.mataPelajaran', 'jadwal.jadwalPelajaran.guru']);
            ...
            // Generate PDF
            $viewName = 'exports.student-class-attendance-pdf';
        }
