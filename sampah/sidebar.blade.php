    ...
    <div class="sidebar-wrapper active">
        ...
        <div class="sidebar-menu">
            <ul class="menu">
                @php
                    $role = Auth::user()->role;
                    $isGuru = $role === 'karyawan' && Auth::user()->karyawan && strtolower(Auth::user()->karyawan->jabatan) === 'guru';
                    $isWaliKelas = $role === 'karyawan' && Auth::user()->karyawan && !empty(Auth::user()->karyawan->kelas_id);
                    $isKurikulum = $role === 'karyawan' && Auth::user()->karyawan && strtolower(Auth::user()->karyawan->jabatan) === 'kurikulum';
                @endphp
                ...
                <li class="sidebar-item has-sub {{ request()->is('admin/absensi-*') ? 'active' : '' }}">
                    ...
                    <ul class="submenu {{ request()->is('admin/absensi-*') ? 'active' : '' }}">
                        <li class="submenu-item {{ request()->is('admin/laporan-absensi-gerbang*') ? 'active' : '' }}">
                            <a href="{{ route($role.'.laporan-absensi-gerbang') }}">
                ...
                <!-- Riwayat Kehadiran -->
                <li class="sidebar-item {{ request()->is('karyawan/kehadiran*') ? 'active' : '' }}">
                    <a href="{{ route('karyawan.attendance.track') }}" class='sidebar-link'>
                ...
                <li class="sidebar-item has-sub {{ request()->is('karyawan/absensi-*') ? 'active' : '' }}">
                    ...
                    <ul class="submenu {{ request()->is('karyawan/absensi-*') ? 'active' : '' }}">
                        ...
                        @if ($isGuru)
                        <li class="submenu-item {{ request()->is('karyawan/absensi-siswa-import*') ? 'active' : '' }}">
                            <a href="{{ route($role.'.absensi-siswa-import') }}">
                                <i class="bi bi-file-earmark-arrow-up me-2"></i>Import Absensi Siswa
                            </a>
                        </li>
                        @endif
                ...
                <!-- Laporan - hanya untuk Wali Kelas dan Kurikulum -->
                @if ($isWaliKelas || $isKurikulum)
                <li class="sidebar-item has-sub {{ request()->is('karyawan/laporan-*') ? 'active' : '' }}">
                    <a href="#" class='sidebar-link'>
                        <i class="bi bi-file-bar-graph-fill"></i>
                        <span>Laporan</span>
                    </a>
                    <ul class="submenu">
                        <li class="submenu-item {{ request()->is('karyawan/laporan-absensi-siswa*') ? 'active' : '' }}">
                            <a href="{{ route($role.'.laporan-absensi-siswa') }}">
                                <i class="bi bi-people me-2"></i>Laporan Absensi Siswa
                            </a>
                        </li>
                        <li class="submenu-item {{ request()->is('karyawan/laporan-absensi-gerbang*') ? 'active' : '' }}">
                            <a href="{{ route($role.'.laporan-absensi-gerbang') }}">
                                <i class="bi bi-door-open me-2"></i>Laporan Absensi Gerbang
                            </a>
                        </li>
                    </ul>
                </li>
                ...
                <li class="sidebar-item {{ request()->is('siswa/riwayat-absensi*') ? 'active' : '' }}">
                    <a href="{{ route($role.'.riwayat-absensi') }}" class='sidebar-link'>
                        <i class="bi bi-clock-history"></i>
