...
<table class="tabel-absensi">
    ...
    <tbody>
        ...
        <tr>
            <td>Absensi Lengkap (Masuk dan Keluar)</td>
            <td>{{ $stats['completeAttendance'] }}</td>
        </tr>
        <tr>
            <td>Belum Absen Keluar</td>
            <td>{{ $stats['incompleteAttendance'] }}</td>
        </tr>
    </tbody>
</table>
        ...
        @if($groupedBySiswa->count() > 0)
            ...
            @foreach($groupedBySiswa as $siswaId => $absensiSiswa)
                @php
                    ...
                    $absensiLengkap = $absensiSiswa->whereNotNull('waktu_scan_keluar')->count();
                    $belumAbsenKeluar = $absensiSiswa->whereNull('waktu_scan_keluar')->count();
                @endphp
                <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $siswa->nis }}</td>
