...
<head>
    <meta charset="UTF-8">
    <title>Laporan Absensi Kelas Guru</title>
    <style>
        body {
...
<h2 style="text-align: center; margin: 20px 0;">LAPORAN ABSENSI KELAS GURU</h2>
...
<table class="tabel-absensi">
    ...
    <tbody>
        ...
        <tr>
            <td>Total Kelas yang Diajar</td>
            <td>{{ $absensi->groupBy('jadwal.kelas_id')->count() }}</td>
        </tr>
        <tr>
            <td>Total Mata Pelajaran</td>
            <td>{{ $absensi->groupBy('jadwal.jadwalPelajaran.mataPelajaran.id')->count() }}</td>
        </tr>
        <tr>
            <td>Total Hari Mengajar</td>
            <td>{{ $absensi->groupBy('tanggal')->count() }}</td>
        </tr>
    </tbody>
</table>
