        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group">
                        <a href="{{ route('karyawan.riwayat-absensi', ['type' => 'gerbang']) }}"
                            class="btn btn-sm {{ $type == 'gerbang' ? 'btn-primary' : 'btn-outline-primary' }}">Absensi
                            Gerbang</a>
                        <a href="{{ route('karyawan.riwayat-absensi', ['type' => 'kelas']) }}"
                            ...
                            <div class="card shadow-sm">
                                ...
                                <div class="card-body">
                                    <div class="d-grid gap-2">
                                        <a href="{{ route('karyawan.riwayat-absensi.export-pdf', ['type' => $type, 'period' => $period, 'start_date' => $customStart, 'end_date' => $customEnd, 'kelas_id' => $kelas->id ?? '']) }}"
                                ...
                                @if ($absensi->count() > 0)
                                    ...
                                    @foreach ($absensi as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->jadwal->jadwalPelajaran->mataPelajaran->nama_mapel ?? 'Tidak tersedia' }}
                                            </td>
                                            <td>{{ $item->jadwal->jadwalPelajaran->guru->nama_lengkap ?? 'Tidak tersedia' }}
                                            </td>
                                            <td>
                                                @if ($item->jadwal)
                                                    {{ substr($item->jadwal->jam_mulai, 0, 5) }} -
                    ...
                    <div>
                        <ul class="pagination pagination-sm">
                            ...
                            @if ($absensi->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $absensi->nextPageUrl() }}" rel="next">&raquo;</a>
                                </li>
                            @else
                                <li class="page-item disabled" aria-disabled="true">
